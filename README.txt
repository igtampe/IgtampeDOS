...
FEATURES:
	...
	-An Extensive amount of software. 30+ Pieces of software Directly available!
	-And some more things i Guess...


For Some Info on Igtampe DOS go to http://igtdos.yolasite.com/


FUTURE UPDATES:
	IGTAMPE DOS:
		No New Goals for Igtampe DOS for this Update.
...
()IGTAMPE DOS
	IGTAMPE DOS LUNA SERVICE PACK 2 (7.2) "... --- ..."
		-Added an Emergency Reccovery System
			-SOS Mode
		...
		-Added a Debug window
			...
			-Launches seperate window which shows what Igtampe DOS Is Doing.
			-CAN WORK WITH APPS.
				-Write information to %DEBUGDIR%.
		-Added Notification and Background Application System
			-Notifications can be sent by writing them to %NOTIFDIR%
			-NABAS will take care of updates now.
				-Checks for a drive and makes sure if it's a higher Build
				 Number before copying to a temporary folder. When you
				 shutdown/hibernate/close Igtampe DOS, It updates the System.
			-Background apps saved in the NABAS Subdirectory can be
			 launched by writing them like this "Echo APP> %NABAS%"
				-Backup has been moved to this system.
		-Added 2 more secrets.
		-Made Igtampe DOS FULLY COMPATIBLE WITH 2000 AND XP
			-VBS is your savior.
		-You can now save a Memory Dump (If not in BETA which is automatic)
		-Backup now writes ID, Version, And build for Restore to identify.
		-IGTINF Now shows what version of windows you are runing instead of just "Microsoft Windows"
			-Thank you WIN variable.
		-Updated Finder to 2.1
			-Not as specific anymore.
		-Fixed bug with secrets to alow them to be seen by XP users.
		-Fixed Bug that would allow the user to do a Direct Boot when the
		 Out Of Box Experience (OOBE) Has not been run.
			-BTW, I'm calling the First Run Menu the OOBE now.
		-FINALLY Fixed Alarms (Version 3.2) (See Bottom of readme)

	IGTAMPE DOS LUNA (UPDATE 1) (7.1) "Finaly, SHORTCUTS!"
		...
		-Added Shortcut System
			-Command String launched from Main Screen programable using the Shortcut Manager,
			 "SHRTCTMAN".
		...
		-Igtampe DOS now knows on which version of windows it's run (Variabe %WIN%)
		...
		-Fixed Bugs:
			...
			-Fixed Alignment bug on one of the screens of regular booting (Not sure
			 if this bug was made on Betas...)
	...
	IGTAMPE DOS LUNA (7.0) "Up in the Sky!"
		...
		-Updated IGTBIOS to 2.4.2
			-Decided to Remove Boot Options
				-Boot Options do NOT work no matter what is modified and it has been decided
				 to remove them. (Besides, Hibernating does an automatic Direct Boot, plus 
				 restores 
		-Removed Texteria
			-If you had a Previous Copy of IGTDOS, it will not be deleted.
		-Removed Sounds for Alarms
			-Alarms Settings Application makes it easier to set a sound instead of coosing from a 
			 limitted Library, removing the need for a sound library in te First Place
		-Fixed Bugs:
			-Fixed Bugs that would make Most Built in Apps and Some Internal commands not
			 Function properly if they were not executed on the installation Drive or if
			 the installation Drive was not C:.
			-Fixed Bug that might make the First Run not Propperly create Folders in the
			...
			-Fixed Bug that would make the First Boot Agent Run again if the System was not
			 restarted on the installation Drive.
			-Fixed bug that would make Igtampe DOS think "*" was an App.
			-Fixed Bug that would make the User return to their DLLFILES directory if there was an
			 Error Updating
			-Fixed bug that would make Igtampe DOS not use the Crash Resistancy if there was an
 			 error launching an app.
			-Fixed bug that would make the user return to their APPS directory if an App failed to
			 Launch.
		-Discovered Things:
			-Timeout Problem from Windows 2000 and XP will not be fixed
				-Timeout is not a valid Application for Windows 2000 and XP
...
()IGTAMPE DOS COMPACT EDITION:
	...
	Igtampe DOS Cell (Update 1) (1.4.1) (1023 Bytes) (JUST under a Kb)
		-Removed TANDC command
		-Compacted some wording in Certain areas.	
...
()ALARMS:
	ALARMS 3.2 "Alarms FINALLY WORKS"
		-Switched to my new SoundPlay API which plays sound in
		 the background with no interuption to the user.
		-Switched to the Message Maker API which makes the pop
		 ups look much better.
		-Fixed Bugs:
			-Fixed bug with the Alarms Settings application
			 that wrote time with an extra space, making it
			 impossible to use.
			-Fixed Bug that made Alarm 7 not function.

	ALARMS 3.1 "Does this really need to be this precise?"
		-Removed the need for Seconds in Time inputted
			-NOT: 13:00:00, NOW: 13:00
